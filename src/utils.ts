      ...
      for (const [key, value] of Object.entries(spec)) {
        ...
        if (
          ...
          if (Array.isArray(value)) {
            const values = [...((prev as any)[key] ?? []), ...value];

            if (key === "tags") {
              (prev as any)[key] = Array.from(new Set(values));
            } else if (key === "parameters") {
              (prev as any)[key] = mergeParameters(values);
            } else {
              (prev as any)[key] = values;
            }
          } else if (typeof value === "function") {
            (prev as any)[key] = value(route);
          } else {
            if (key === "parameters") {
              // @ts-expect-error
              (prev as any)[key] = mergeParameters((prev as any)[key], value);
            } else {
              (prev as any)[key] = {
                ...(prev as any)[key],
                ...value,
...
export function removeExcludedPaths(
  ...
  for (const [key, value] of Object.entries(paths)) {
    ...
    for (const method of Object.keys(value)) {
      const schema = value[method as keyof OpenAPIV3_1.PathItemObject] as any;
    ...
    for (const method of Object.keys(value)) {
      const k = method as keyof OpenAPIV3_1.PathItemObject;
      if (value[k] != null) {
        (filteredValue as any)[k] = value[k];
      }
    }
